...
# moved json import to json functions
import sys
import threading
# removed yaml import b/c want ot use json
...
class ROVAction(str, Enum):
    ...
    AXIS_CLAW_OPEN = "claw_open"
    AXIS_CLAW_ROTATE = "claw_rotate"
    AXIS_CLAW_TILT = "claw_tilt"
    AXIS_CAMERA = "camera"

    BUTTON_FORWARD = "forward"
    BUTTON_STRAFE = "strafe"
    BUTTON_YAW = "yaw"
    BUTTON_HEAVE = "heave"
    BUTTON_ROLL = "roll"
    BUTTON_PITCH = "pitch"

    BUTTON_CLAW_OPEN = "claw_open"
    BUTTON_CLAW_ROTATE = "claw_rotate"
    BUTTON_CLAW_TILT = "claw_tilt"
    ...
    def prompt(self) -> str:
        ...
        prompts = {
            ROVAction.FORWARD:
                "Move the control you want to use for FORWARD in the forward direction.",
            ROVAction.STRAFE:
...
class OldJoystickMapper(Node):
    ...
    def __init__(self) -> None:
        ...
        self.spin_command_prompt_thread = threading.Thread(target=self.spin_command_prompt, daemon=True,)
        self.spin_command_prompt_thread.start()

        self.discovery_timer = self.create_timer(self.discovery_period_sec,self._refresh_joy_subscriptions,)
